...
const writeFile = require('@iorp/node-aid/src/filesystem/writeFile');
const writeEnvFile = require('./writeEnvFile');
...
function initEnv(environmentName, environments, environmentToPackageKeys = null, callback = null, basePath = null, verbose = false) {
  try {
    ...
    if (!environment) throw {
      'error': 1,
      'code': 'DevjsWrongEnvironmentName',
      'exception': `There is no environment named that way.`
    };
    ...
    if (environmentToPackageKeys) {
      ...
      r = writeFile(packagePath, environmentToPackageKeys, {
        datatype: 'JSON',
        merge: true
      });
